...
def is_ultrametric(tree):
    ...
    Uses min/max root-to-tip distance comparison.
    """
    root = tree.treenode
    ...
    while stack:
        ...
        if node.is_leaf():
            ...
            for child in node.children:
                child_dist = dist + (child.dist if child.dist else 0.0)
                stack.append((child, child_dist))

    return (max_dist - min_dist) <= 1e-6
...
# Print tree info
print("\nTest tree leaf count:", len(trs.get_tip_labels()))
...
def load_newick_trees(filepath):
    """
    Load Newick trees into toytree, using ete3 as a robust intermediate parser.
    This avoids toytree's fragile internal tree initialization on certain inputs.
    """
    trees = []
    with open(filepath, "r") as f:
        for line_num, line in enumerate(f, start=1):
            ...
            if not line:
                continue
            try:
                # Step 1: parse with ete3 (more robust)
                ete_tree = Tree(line, format=1)
                # Step 2: re-export normalized Newick
                clean_newick = ete_tree.write(format=1)
                # Step 3: load into toytree
                trees.append(toytree.tree(clean_newick))
            except Exception as e:
                print(f"Skipping tree at line {line_num}: {e}")
                print(f"  Bad line preview: {line[:200]}")
    return trees


ultra1k = load_newick_trees("ultra1k.nwk")
print(f"Loaded {len(ultra1k)} trees")

hetero1k = load_newick_trees("hetero1k.nwk")
print(f"Loaded {len(hetero1k)} trees")
