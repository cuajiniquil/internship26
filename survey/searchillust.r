...
#report: 
plot(tr)
nodelabels(c("9","5","3","8"), 6:9, adj = -1, bg = NA, frame = "none") 
...
plotpo <- ggtree(tr) %<+% custom_labels +
  geom_text2(aes(label = labpo), hjust = -1, color = "darkgreen", size = 6) +
...
colorpo <- function(nodes, color) {
  list(
  geom_text2(aes(label = labpo, subset = labpo %in% nodes), color = color, hjust = -1, size = 6),
  geom_point2(aes(subset = labpo %in% nodes), color = color, size = 3)
  )
}
