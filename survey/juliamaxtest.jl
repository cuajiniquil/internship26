...
function running_max(times::Vector{Float64})::Vector{Float64}
    """Return cumulative maximum of a time series.
    running_max([12, 2, 23]) => [12.0, 12.0, 23.0]
    """
    result = similar(times)
    current_max = 0.0
...
parsedu1k = parsenewick(String(ultra1k[1]))
test_tree = parsedu1k
...
sstart = time_ns()
sres_preorder    = running_max(benchmark_sys(preorder_fn, 1000))
sres_postorder   = running_max(benchmark_sys(postorder_fn, 1000))
sres_levelorder  = running_max(benchmark_sys(levelorder_fn, 1000))
sres_ultrametric = running_max(benchmark_sys(ultrametric_fn, 1000))
send = time_ns()
...
for (method_name, method_times) in [
    ...
    println("  Max at index:  $(argmax(method_times))")
end
