...
function main()
    ...
    # Correct closures: each takes a tree as input
    intervals_times = benchmark_sys(hetero1k, tree -> extract_coalescent_intervals(tree), 1000)
    loglik_times    = benchmark_sys(hetero1k, tree -> loglik_phylo(tree), 1000)

    println("\n=== Phylo.jl Benchmark Statistics (Coalescent likelihood) ===")
    for (method_name, method_times) in [
        ("interval extraction", intervals_times),
        ("log likelihood", loglik_times)
    ]
        println("\n$method_name:")
    ...
    data_by_method = [intervals_times, loglik_times]
    positions = [1, 2]
    ax.violinplot(data_by_method, positions=positions, widths=0.7)
    ax.set_ylabel("Time (microseconds)")
    ax.set_title("Phylo.jl Coalescent Likelihood Computation")
    ax.set_xticks(positions)
    ax.set_xticklabels(["interval ext.", "log likeli."])
    ax.grid(true, axis="y", alpha=0.3)

    tight_layout()
    savefig("phylo_jl_coal_likelihood.png", dpi=100)
    println("\nPlot saved to 'phylo_jl_coal_likelihood.png'")
    close()
end
